...
2. **Zotero first, network fallback.** For each claim with `STATUS: stub`:
   ...
   - Zotero miss (DOI + semantic, or disabled) → invoke `Skill(skill="research-lookup")` / `Skill(skill="citation-management")`. On hit: `source: network`; if `zotero.auto_push_new_citations: true`, call `zotero_add_by_doi(doi=<DOI>, collection_key=<key>)` (the tool dedups by DOI) and mark `source: both`.
