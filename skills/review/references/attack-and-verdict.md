...
This file holds the long detail for the review skill's `adversarial` mode: the attack and adjudication briefs, the six attack axes, the rejection-dimension checklist, and the count-to-verdict table the helper implements. Use each brief once in the Codex lane and once in the Hermes lane. The adjudication in each lane receives only that provider's attack. Run the verdict helper separately for each provider.
