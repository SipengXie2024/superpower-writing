...
2. **Optional paired external opinions.** For a contentious or high-stakes idea, run Codex and Hermes together before locking the local verdict. Send both the same dossier through `${CLAUDE_PLUGIN_ROOT}/scripts/paired_consult.py` with `--handoff-kind novelty`. Keep the local retrieval result, Codex view, and Hermes view separate. The dossier protocol and briefing template are in [`references/cross-model-protocol.md`](references/cross-model-protocol.md). This step is optional, but once enabled it always calls both providers. Skip it for a clearly novel or clearly dominated idea where verified prior work already settles the question.
