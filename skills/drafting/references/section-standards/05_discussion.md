...
- **Tense:** mixed tense is standard. Use **present tense** for what results mean, interpretations, and generalizable claims ("These findings suggest that…"; "This pattern indicates…"). Use **past tense** for what was done and observed ("We found that…"; "The 3-point gain came from…"). Use **future tense** only in the Future work element ("Future studies should investigate…").
...
- **Interpretation-bounded claims.** Every claim in `[I]` and `[S]` should trace back to a specific §Results finding. If a claim cannot be traced, it is speculation without evidence and should be weakened with "may", "might", or "we hypothesize".
- **Limitations should be self-aware, not self-deprecating.** The goal is not to list every possible weakness but to identify the 2--3 most impactful threats to validity and explain how they affect the conclusions. Over-listing trivial limitations signals insecurity; under-listing real ones signals carelessness.
- **Future-work concreteness test.** Each `[F]` item should pass the "next-paper test": can a reader scope a follow-up paper from this item alone? If not, the item is too vague.
- **Significance element answers "so what?".** `[S]` is the place to spell out who benefits from this work and how. Skip generic statements ("this advances the field"); name the concrete beneficiary (practitioners, system designers, researchers in subfield X) and the concrete benefit (reduced deployment cost, new capability, faster iteration).
...
- **Overclaiming significance.** `[S]` extrapolates beyond what the evidence supports ("This work solves the scalability problem for all distributed systems"). Symptom: reviewer flags as overclaim; fairness scores drop. Fix: calibrate every significance claim against what §Results actually establishes; use "suggests", "indicates", or "enables" rather than "proves", "solves", or "revolutionizes".
- **Ignoring threats to validity.** The `[L]` element is missing or consists only of vague hedges ("our approach has limitations"). Symptom: single most common reviewer complaint in CS ("limitations not adequately discussed"). Fix: list 2--3 specific, concrete threats --- untested settings, missing baselines, assumptions that may not hold, scope boundaries.
- **Restating results verbatim.** Discussion paragraphs copy sentences from §Results with minor wording changes instead of interpreting. Symptom: reviewer writes "the discussion adds nothing beyond the results". Fix: every Discussion paragraph should answer "why" or "what does this mean", not "what was the number".
- **No comparison to prior work.** Discussion presents interpretations in isolation without connecting to the existing literature. Symptom: reviewer writes "how do these findings relate to prior work?". Fix: every `[I]` interpretation should reference at least one prior result ("Consistent with Smith et al., we find…" or "Contrary to Jones et al., our results suggest…").
- **Ignoring negative or unexpected results.** Unfavorable findings are silently omitted from Discussion. Symptom: reviewer notices the gap between what was measured and what is discussed. Fix: explicitly address unexpected results; negative findings often yield the most informative interpretations.
- **Significance element missing.** Discussion ends after future work without answering "so what?". Symptom: reader finishes the paper unsure what the contribution unlocks. Fix: always include at least one `[S]` paragraph spelling out who benefits and how.
- **Over-generalization beyond evaluation scope.** `[S]` claims the approach works for all domains when it was only tested on one. Symptom: reviewer writes "this claim is not supported by the experiments". Fix: scope significance claims to the evaluated settings; use "suggests" for extrapolations.
- **Mechanistic explanation without evidence.** `[I]` proposes a causal mechanism ("the improvement is due to better gradient flow") without pointing to any supporting evidence (ablation, diagnostic experiment, prior literature). Symptom: reviewer writes "this interpretation is speculative". Fix: every mechanistic claim needs at least one empirical anchor (from this paper's ablations or from cited prior work).
- **Limitation section too defensive.** `[L]` reads as a pre-emptive rebuttal rather than an honest assessment. Symptom: reviewer writes "limitations are acknowledged but not taken seriously". Fix: state limitations plainly, then explain how they affect the conclusions. Do not immediately follow every limitation with a defense.
- **Repetitive future-work items.** `[F]` lists three variations of the same follow-up ("apply to domain A", "apply to domain B", "apply to domain C"). Symptom: reviewer sees padding. Fix: consolidate related directions into one item with a list of example domains; reserve separate items for methodologically distinct follow-ups.
- **Discussion too short.** §Discussion is 2--3 sentences that merely restate the abstract. Symptom: reviewer writes "the discussion is superficial" or "no depth of analysis". Fix: expand `[I]` to address each RQ's mechanism; add concrete `[L]` threats. Target at least 800 words for a full paper.
- **Unstructured discussion.** Paragraphs are written without the ILFS framework, mixing interpretation, limitations, and future work in the same paragraph. Symptom: reviewer cannot find the limitations section. Fix: use explicit subsections or clear paragraph-level tags for I, L, F, S elements.
