...
description: Create publication-quality scientific diagrams using Nano Banana 2 AI with smart iterative refinement. Uses Gemini 3.1 Pro Preview for quality review. Only regenerates if quality is below threshold for your document type. Specialized in neural network architectures, system diagrams, data-flow pipelines, and complex scientific visualizations.
...
python scripts/generate_schematic.py "Distributed transaction processing pipeline: client request through load balancer, consensus module, write-ahead log, and storage backend" -o figures/txn_pipeline.png --doc-type journal

# Generate for presentation (lower threshold: 6.5/10 - faster)
python scripts/generate_schematic.py "Transformer encoder-decoder architecture showing multi-head attention" -o figures/transformer.png --doc-type presentation

# Generate for poster (moderate threshold: 7.0/10)
python scripts/generate_schematic.py "Multi-tier cache hierarchy diagram: L1, L2, shared L3, DRAM controller, and NVMe storage layers" -o figures/cache_hierarchy.png --doc-type poster
...
- "Pathway diagram" (which pathway? what components?)
...
```json
{
  "user_prompt": "Multi-stage data processing pipeline: ingest → validate → transform → aggregate → store...",
  "doc_type": "poster",
  "quality_threshold": 7.0,
  "iterations": [
    {
      "iteration": 1,
      "image_path": "figures/pipeline_v1.png",
      "score": 7.5,
...
python scripts/generate_schematic.py \
  "Distributed transaction processing pipeline architecture diagram. \
...
```bash
python scripts/generate_schematic.py \
  "Multi-tier cache hierarchy diagram for a multi-core processor. \
