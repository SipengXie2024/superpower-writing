# Novelty Delta Rubric (Phase 2)

How to score each core claim HIGH, MED, or LOW against its closest prior work, how to apply the non-obvious novelty rules, and how to aggregate per-claim deltas into an overall call. The verdict is advisory. It ranks and explains, it does not gate execution.

## 1. Per-claim delta tiers

Score every core claim against the single closest prior work found in the search. Use MED as the default. Justify any move up or down with a specific overlap note.
...
Map the set of per-claim deltas to one of three calls. A claim is **load-bearing** when the idea collapses without it. This map is a reading aid, not a mechanical switch. The user owns the decision.
...
- If two or more load-bearing claims are LOW, lean toward ABANDON even when one claim is MED. A published baseline already dominates the core of the idea.
...
- **Verdict-delta mismatch**: an overall PROCEED with no claim above MED reads as incoherent. Realign the verdict with the table.
