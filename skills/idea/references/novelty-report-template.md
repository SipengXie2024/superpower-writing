# Novelty Report Template (Phase 2)

The fixed output structure for the novelty verdict, plus a worked example. The output is advisory. It is surfaced to the user and, on request, saved as `.writing/novelty-report.md`. It never mutates claim STATUS or any existing `.writing/` file.
...
- **Core claims table**: one row per claim, three to five rows. Novelty is HIGH, MED, or LOW from the rubric. Closest prior work is a verified paper, an `[UNVERIFIED]` candidate, or `none found`. Never blank.
...
When the user wants the output saved and a `.writing/` directory exists, write it to `.writing/novelty-report.md`. It adds no claim STATUS and edits no existing file. Ask before writing. Do not create `.writing/` solely to hold the file. When no project state exists, deliver the output inline.
