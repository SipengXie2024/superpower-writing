# Generative Lens Catalog (Phase 1)
...
The lenses produce breadth. They do not judge quality. A candidate's worth is decided later by the FINER scores and any cross-model pass, never by the lens that produced it.
...
Ground the generation in real prior work. Delegate targeted searches per lens to the retrieval skill so candidates rest on actual papers, not memory. Never fabricate a paper, an arXiv ID, or a DOI to justify a candidate. Mark an unconfirmed reference `[UNVERIFIED]` and move on.

Do not critique while generating. Capture every candidate, even the ones that feel weak. Weakness is a downstream reading. Dropping a candidate at generation time defeats the breadth the lenses exist to create.
