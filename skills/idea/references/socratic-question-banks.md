# Socratic Question-Banks And Strategies (Phase 1 opt-in)

This reference powers the opt-in Socratic coaching mode of Phase 1. In that mode the skill does NOT produce a research-direction brief and does NOT score FINER for the user. It drives the user to derive the direction themselves through questions.
...
The mode follows a convergence limit. Aim to converge within about 15 rounds. If the user cannot converge after several rounds in problem framing, offer to switch to the default generate-and-evaluate path rather than looping forever.
...
- Aim to converge within about 15 rounds. If the user stalls, offer to switch to the default generate-and-evaluate path.
- If the user asks for a direct answer, gently decline and explain that the value here is in deriving the direction themselves. The user may always exit to the default path if they prefer to be handed candidates.
...
If the user then wants a scored brief, they can switch to the default path. The default path reads this summary and starts from FINER scoring rather than from a blank page.
