...
- **"Review my paper", "get an external review", "second opinion", "mock venue review"** → `superpower-writing:external-review`. It runs Codex and Hermes concurrently through `${CLAUDE_PLUGIN_ROOT}/scripts/paired_consult.py`, preserves both views separately, and returns venue reviews, results-to-claims matrices, or experiment plans. Verdict-bearing, so never wrap it in `/loop`, `/schedule`, or `CronCreate`.
- **"Ask Codex", "ask Hermes", "consult another model"** → load `superpower-writing:collaborating-with-codex` and `superpower-writing:collaborating-with-hermes`, then use the shared paired consultation route for read-only academic work. Scientific figure generation is the only Codex-only consultation exception.
...
| `superpower-writing:external-review` | Stable draft / idea / result → independent Codex and Hermes critiques through `paired_consult.py`. Keeps both reviews separate and returns mock reviews, results-to-claims matrices, and experiment plans. Advisory; verdict-bearing, never wrap in `/loop`. |
| `superpower-writing:collaborating-with-codex` | Codex academic consultation, authorized direct execution, session continuity, and the Codex-only scientific figure exception. |
| `superpower-writing:collaborating-with-hermes` | Hermes academic consultation, stateless follow-ups, workspace-change detection, and authorized direct execution. |
