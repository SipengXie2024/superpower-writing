...
Wrapping this external-review skill in `/loop 30m` re-runs a verdict-bearing skill on a clock that has nothing to do with whether the artifact changed. Zero new signal, full token cost. Worse, an external `/loop` re-enters the skill from the top each tick and loses provider-specific review history. "Did you fix the gap I named?" becomes unanswerable because the prior Codex session and Hermes handoff are not carried into the new tick.
